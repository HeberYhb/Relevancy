...
def loadSimpDat():
    ...
    simpDat = [['r', 'z', 'h', 'j', 'p'],
               ...
               ['y', 'z', 'x', 'e', 'q', 's', 't', 'm']]
    '''
    testDt = pd.read_csv('./data/LogAlarm_testData.csv', header=0)
    timeGroups = testDt['事件摘要'].groupby(testDt['首次发生日期'])
    testDt_List = []
    for group in timeGroups:
        # group[1].to_csv(str(group[0]) + '.csv', index=False, encoding='UTF-8')
        testDt_List.append(np.array(group[1]).tolist())
    return testDt_List
